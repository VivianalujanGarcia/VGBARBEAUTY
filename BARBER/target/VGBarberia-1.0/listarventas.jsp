    ...
    <body>
        <div class="container">
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
                <strong> No se puede ingresar a la factura venta porque no hay una caja abierta.</strong>
            </div>
        </div>
        <script>
...
<div class="content-wrapper">
    <section class="content">
        <h1>Listar Ventas <button class="btn btn-outline-secondary" data-ciudades="1" title="Imprimir" data-toggle="modal" data-target="#Imprimirl">
    Imprimir Listado General <img src="img/impresora (1).png" title="Imprimir"></h1>
        <button type="button" onclick="location.href = 'detventas.jsp'" class="btn btn-primary">Nueva Venta</button>
        <table class="table">
            <thead>
                <tr>
                    <th scope="col">#</th>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="eliminar-registro-venta">SI</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    // Función para asignar eventos de clic a los botones de eliminar venta
    function asignarEventosEliminarVenta() {
        $(".eliminar-venta").click(function () {
            var idVenta = $(this).data("id");
            $("#idpk").val(idVenta); // Establecer el id de la venta en el campo oculto del modal
        });
        
    $("#imprimirtodo").on('click', function () {
        var pk = $("#pkim").val();
        window.open('jsp/reporteventas.jsp?pkim=' + pk, '_blank');
        $.ajax({
            data: {listar: ' ', pkim: pk},
            url: 'jsp/insercionventas.jsp',
            type: 'post',
    ...
    $("#imprimi").on('click', function () {
        var pkim = $("pkim").val();
        window.open('jsp/ListadoVentas.jsp?pkim=' + pkim, '_blank');
        ...
        // Evento para confirmar la eliminación de la venta
        $("#eliminar-registro-venta").click(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    listarventas(); // Actualizar la lista de ventas después de eliminar
